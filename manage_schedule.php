...
if (isset($_GET['id']) && !empty($_GET['id'])) {
    $id = intval($_GET['id']); // Prevent SQL injection and warning
    $qry = $conn->query("SELECT * FROM schedule_list WHERE id = $id")->fetch_array();
        ...
        <div class="col-md-12">
            <div class="form-group mb-2">
                ...
                <select name="bus_id" id="bus_id" class="form-control" required>
                    ...
                    <?php while ($row = $bus->fetch_assoc()) { ?>
                        <option value="<?php echo $row['id'] ?>" <?php echo isset($meta['bus_id']) && $meta['bus_id'] == $row['id'] ? 'selected' : '' ?>>
                            <?php echo $row['bus_number'] . ' | ' . $row['name'] ?>
                        </option>
                    <?php } ?>
                </select>
            </div>
            ...
            <div class="form-group mb-2">
                ...
                <select name="from_location" id="from_location" class="form-control" required>
                    <option value="" <?php echo isset($meta['from_location']) && $meta['from_location'] > 0 ? '' : 'selected' ?> disabled="">Select Here</option>
            ...
            <div class="form-group mb-2">
                <label for="availability" class="control-label">Available Seats</label>
